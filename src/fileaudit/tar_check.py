...
def _validate_url_scheme(path_str: str) -> bool:
    ...
    if parsed.scheme:
        ...
        # Protocol-relative URLs like //example.com/file.tar
        raise TarValidationError(
...
def validate_tar(func_or_path=None, 
                 max_file_size=None,
    ...
    1. **Decorator mode** — wraps a function to validate a TAR file path
    ...
    Usage:
        @validate_tar
        @validate_tar()
        @validate_tar("custom_arg_name", max_file_size=5000)
        validate_tar("path/to/file.tar", max_tar_members=100)  # CLI usage
        validate_tar("https://example.com/file.tar")           # HTTPS only
...
def _validate_tar_file(path, max_file_size, max_tar_members, 
                       max_total_extracted_size, max_individual_file_size,
                       max_filename_length, max_directory_depth):
    """
    Internal validation function for TAR files.
    
    Performs all security checks on a TAR file.
    """
    path_str = str(path)
    ...
    try:
        if is_remote:
            ...
            with _https_opener.open(req, timeout=DOWNLOAD_TIMEOUT) as response:
                tar_data = response.read(max_file_size + 1)
                if len(tar_data) > max_file_size:
                    raise TarValidationError(
                        f"Remote file exceeded maximum size "
    ...
    try:
        # Use a temporary file for large archives to avoid memory issues
        ...
        try:
            ...
            try:
                ...
                # Otherwise, raise so the caller knows cleanup failed.
                if sys.exc_info()[0] is None:
                    raise

    
    except Exception as e:
        raise TarValidationError(f"Failed to process TAR archive: {e}") from e